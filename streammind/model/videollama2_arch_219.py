...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos_score_cls_video_cls_autoregressive(self, images_or_videos,cls_inference = False,cls_training = False,caption_info = None):
        ...
        for idx, images_or_video in enumerate(images_or_videos):
            ...
            if frames.shape[0] > 600:
                frames = frames[-600:]
                # zero_frames = zero_frames[-600:]
            frames_features = self.get_model().get_vision_tower()(frames)#self.get_model()-> return self.model,使用clip做image encode,frames_features:[num_frames*batchsize,576,1024] [512,576,1024]
            # frames_features_zero = self.get_model().get_vision_tower()(zero_frames)#self.get_model()-> return self.model,使用clip做image encode,frames_features:[num_frames*batchsize,576,1024] [512,576,1024]
            # import pdb
            # pdb.set_trace()
            # frames_features = torch.cat([frames_features,frames_features_zero],dim = 0)
    ...
    def temporal_aggregator(self, frames_features,cls_demo = False,cls_inference = False,cls_training = False,caption_info = None,frames_features_shape=None,tokenizer = None):
    ...
    def prepare_inputs_labels_for_multimodal_score_stream(self, input_ids, attention_mask, past_key_values, labels, X_modalities,timestamp,**kwargs):
        ...
        exactor_output,feature_idx = self.encode_images_or_videos_score_cls_video_cls_autoregressive(Xs,
                                                                                                ...
                                                                                                cls_inference=cls_inference)#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        # import pdb
        # pdb.set_trace()
