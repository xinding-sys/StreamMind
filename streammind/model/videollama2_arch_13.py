...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos_score_cls(self, images_or_videos,past_caption=None):
        # import pdb
        # pdb.set_trace()
        num_frames = images_or_videos.shape[0]
        videos = images_or_videos.unsqueeze(0)
        ...
        frames_features = self.get_model().get_vision_tower()(frames)#self.get_model()-> return self.model,使用clip做image encode,frames_features:[num_frames*batchsize,576,1024] [512,576,1024]
        frames_features = einops.rearrange(frames_features, '(b t) n h -> b t n h', b = batch_size)#[batch:16,num_frames:32,576,1024]
        return self.temporal_aggregator(frames_features,cls = True,past_caption = past_caption)#这个就是过那个connector，见下


    # def encode_images_or_videos_score_cls_video(self, images_or_videos,past_caption=None,cls = False):
    #     # import pdb
    #     # pdb.set_trace()
    ...
    #     if cls :
    #         #需要建立attention_mask 和label
    #         #  
    #     frames_features,cls_feature = self.temporal_aggregator(frames_features,cls = cls,past_caption = past_caption)

    #     return frames_features, frames_features_shape,cls_feature


    def encode_images_or_videos_score_cls_video_cls_autoregressive(self, images_or_videos,past_caption=None,cls = False):
        # import pdb
        # pdb.set_trace()
        frames_features_list = []
        frames_features_shape = []
    ...
    def encode_images_or_videos_score_cls_inference(self, images_or_videos,past_frames_features):
        # import pdb
        # pdb.set_trace()
        num_frames = images_or_videos.shape[0]
        videos = images_or_videos.unsqueeze(0)
        ...
        X_features, cls_feature = self.temporal_aggregator(frames_features,cls = True)

        return X_features, cls_feature,frames_features#这个就是过那个connector，见下




    def encode_images_or_videos_score_cls_inference_allframe(self, images_or_videos, past_frames_features, past_caption=None):
        # import pdb
        # pdb.set_trace()
        num_frames = images_or_videos.shape[0]
        videos = images_or_videos.unsqueeze(0)
        ...
        X_features, cls_feature = self.temporal_aggregator(frames_features, cls = True, past_caption = past_caption)

        return X_features, cls_feature,frames_features#这个就是过那个connector，见下

    def encode_images_or_videos_score_cls_inference_allframe_parallel(self, images_or_videos, past_frames_features, past_caption=None):
        # import pdb
        # pdb.set_trace()
        num_frames = images_or_videos.shape[0]
        videos = images_or_videos.unsqueeze(0)
    ...
    def temporal_aggregator(self, frames_features,cls = False,past_caption = None):
        ...
        # *********** time  ************
            ...
            video_features = self.get_model().mm_projector(frames_features,cls,past_caption = past_caption)
        else:
    ...
    def prepare_inputs_labels_for_multimodal_score_stream(self, input_ids, attention_mask, past_key_values, labels, X_modalities,timestamp,**kwargs    ):
        # import pdb
        # pdb.set_trace()
        ...
        # X_features = self.encode_images_or_videos(Xs, keys)
        # X_features, cls_feature = self.encode_images_or_videos_score_cls(Xs,past_caption =self.get_model().embed_tokens(kwargs["past_review_caption"]) )#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        # import pdb
        # pdb.set_trace()
        X_features,feature_idx,_ = self.encode_images_or_videos_score_cls_video(Xs,past_caption = None,cls=True)#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        start_feature_idx = [0] + feature_idx[:-1]

        # X_features, cls_feature = self.encode_images_or_videos_score_cls_video(Xs,past_caption = None,cls=True)#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        #cls_feature:[eos, no eos]

        # if kwargs["caption_info"] == "</s>":
        #     #说明要用cls loss
        #     # cls_target = torch.tensor([1, 0], dtype = cls_feature.dtype, device = cls_feature.device).unsqueeze(0)
        #     cls_target = torch.tensor([0], dtype = torch.long, device = cls_feature.device)
        #     cls_loss = self.loss_fct(cls_feature, cls_target)
        #     # cls_loss = self.focal_loss(cls_feature, cls_target)
        #     # return None, None, None, None, None, cls_loss
        # else:
        #     # cls_target = torch.tensor([0, 1], dtype = cls_feature.dtype, device = cls_feature.device).unsqueeze(0)
        #     cls_target = torch.tensor([1], dtype = torch.long, device = cls_feature.device)
        #     cls_loss = self.loss_fct(cls_feature, cls_target)
        #     # cls_loss = self.focal_loss(cls_feature, cls_target)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # X_index_inonesample = 0
            while X_token_indices.numel() > 0:
                cur_X_features =self.exponential_sampling(X_features[0][start_feature_idx[cur_X_idx]:feature_idx[cur_X_idx]], 0.7)
    ...
    #     # import pdb
    #     # pdb.set_trace()
    #     X_features,feature_idx,cls_feature = self.encode_images_or_videos_score_cls_video(Xs,past_caption = None,cls=True)#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
    #     start_feature_idx = [0] + feature_idx[:-1]
    ...
    def prepare_inputs_labels_for_multimodal_score_stream_inference_parallel(
         self, input_ids, attention_mask, past_key_values, labels, X_modalities,**kwargs    ):
        vision_tower = self.get_vision_tower()
        # NOTE: text-only situation
        # import pdb
        ...
        X_features, cls_feature ,frames_features,interval_id = self.encode_images_or_videos_score_cls_inference_allframe_parallel(Xs,kwargs["frames_features"])#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
