...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos_score_cls_video(self, images_or_videos,past_caption=None,cls = False):
        ...
        frames_features = self.temporal_aggregator(frames_features,cls = cls,past_caption = past_caption)

        return frames_features, frames_features_shape
    ...
    def prepare_inputs_labels_for_multimodal_score_stream(self, input_ids, attention_mask, past_key_values, labels, X_modalities,timestamp,**kwargs    ):
        ...
        # import pdb
        # pdb.set_trace()
        X_features,feature_idx = self.encode_images_or_videos_score_cls_video(Xs,past_caption = None,cls=False)#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
