...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if self.score_dataset:
            ...
            self.half_dict = dict()
            self.video_id_to_caption_path = dict() #caption_id = video_id
            self.caption_path_to_video_id = dict()
            self.caption_num = 0
    ...
    def preprocess_caption(self,video_data_path,video_path_id):
        ...
        self.video_id_to_caption_path[video_path_id] = caption_data_path
        self.caption_path_to_video_id[caption_data_path] = video_path_id
        if video_path_id == 0:
        ...
        for annotation in data.get('annotations', []):
            ...
            minutes, seconds = map(int, _.split(':'))
            timestamp = minutes * 60 + seconds
            self.caption_dict[video_path_id].append(annotation.get('anonymized', ''))
            self.timestamp_dict[video_path_id].append(timestamp)
            self.half_dict[video_path_id].append(half)
            self.caption_num += 1
            self.caption_num_pervideo[video_path_id] += 1
        self.timestamp_dict[video_path_id] = self.timestamp_dict[video_path_id][::-1]
        self.caption_dict[video_path_id] = self.caption_dict[video_path_id][::-1]
    ...
    def process_score_video(self,timestamp,processor,video_path,cur_fps = 2):
        ...
        # print(len(images),start_frame,timestamp,timestamp*25,end_frame,duration)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if self.score_dataset:
            ...
            video = self.process_score_video(video_path=video_path,timestamp=timestamp, processor=video_processor,cur_fps=2)
            if video is None:
                i = random.randint(0,self.caption_num - 1)
                return self.__getitem__(i)
