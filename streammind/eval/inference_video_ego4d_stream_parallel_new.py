...
def model_init(model_path,model_base = None,model_name="VideoLLaMA2-7B",**kwargs):
...
def run_inference_timediff_fluency_ppl_metric(args):
    ...
    cognition_sample = {"sample_per":args.sample_per,"sample_type":args.sample_type}
    model, processor, tokenizer, version = model_init(args.model_path, model_base = args.model_base, model_name = args.model_name,**cognition_sample)
    ...
    if args.eval_type == "llm":
        ...
        with open(args.caption_path, mode="w", newline="", encoding="utf-8") as file:
                pass  # 直接 pass，不写入任何内容
        with open(args.caption_path, mode="w", newline="", encoding="utf-8") as file:
            writer = csv.writer(file)
            writer.writerow(["start csv log!"])
        for i, inputs in enumerate(tqdm(llm_val_loader)):
            # with open(args.caption_path+"/output.csv", "a") as file:  # 使用 "a" 模式以追加内容
            #             file.write("video_path:{},timestamp:{} \n".format(inputs["video_path"],inputs["timestamp"]))  # 每次写入一行并换行
            lm_ppls = []
            lm_correctness = []
            ...
            for idx,turn in enumerate(turns):
                ...
                # import pdb
                # pdb.set_trace()
                caption_token_id = turn_logit.argmax(dim=-1)
                caption_token = tokenizer.batch_decode(caption_token_id.unsqueeze(0), skip_special_tokens=True)[0].strip()
                caption_target_token = tokenizer.batch_decode(turn_label.unsqueeze(0), skip_special_tokens=True)[0].strip()
                # with open(args.caption_path+"/output.txt", "a") as file:  # 使用 "a" 模式以追加内容
                    # file.write("our caption:{},target:{} \n".format(caption_token,caption_target_token))  # 每次写入一行并换行
                with open(args.caption_path, mode="a", newline="", encoding="utf-8") as file:
                    writer = csv.writer(file)
                    writer.writerow([caption_token, caption_target_token])

                # print(caption_token,666,caption_target_token)
                pred_caption_list[caption_id] = [caption_token]
                target_caption_list[caption_id] = [caption_target_token]
                ...
                correct_token_num.append(num_lm_correct_tokens)
        ...
        print("llm_evaluate_metric_video_token_total",sum(video_token_total)/len(video_token_total))
        with open(args.caption_path, mode="a", newline="", encoding="utf-8") as file:
            writer = csv.writer(file)
            writer.writerow([sum(video_lm_correctness)/len(video_lm_correctness),
                             sum(video_lm_ppls)/len(video_lm_ppls),
                             sum(video_lm_correctness_token)/len(video_lm_correctness_token),
                             sum(video_token_total)/len(video_token_total)])
    elif args.eval_type == "cls":
        # evaluate perception gate
        ...
        for i, inputs in enumerate(tqdm(cls_val_loader)):
            ...
            for idx,_ in enumerate(labels):
                if labels[idx] == 0 :
                    labels_cache.append(labels[idx])
                    logits_cache.append(logits[idx])
                else:
                    labels_cache.append(labels[idx])
                    logits_cache.append(logits[idx])
                    logits_list.append(torch.cat(logits_cache))
                    labels_list.append(torch.cat(labels_cache))
                    logits_cache = []
                    labels_cache = []
            logits = torch.nn.utils.rnn.pad_sequence(logits_list,batch_first=True,padding_value=0).squeeze(-2)
            labels = torch.nn.utils.rnn.pad_sequence(labels_list,batch_first=True,padding_value=IGNORE_INDEX).squeeze(-1)
            ...
            # pdb.set_trace()
            softmax = nn.Softmax()
            eos_logits = softmax(eos_logits)
            pred_labels = eos_logits.argmax(dim = -1)
            # pred_labels = (eos_logits[:, 1] > 0.2).long()
