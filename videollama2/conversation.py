...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            for i, (role, message) in enumerate(messages):
                ...
                if message:
                    ...
                    if i == 0: message = wrap_sys(self.system) +"Please describe the video content in detail based on the provided information." + message
                    if i % 2 == 0:
        ...
        elif self.sep_style == SeparatorStyle.LLAMA_2_LIVE:
            wrap_sys = lambda msg: f"<<SYS>>\n{msg}\n<</SYS>>\n\n"
            wrap_inst = lambda msg: f"[INST] {msg} [/INST]"
            ret = ""

            #需要检查一次，因为有些dialogue是USER，USER ASSIS，这样要把前面的两个USER合并在一起
            def merge_user_messages(conversation):
                merged_conversation = []
                buffer = None
                ...
                for speaker, message in conversation:
                    if speaker == "USER":
                        if buffer is None:
                            ...
                            buffer += " " + message  # 合并连续的 USER 消息
                    else:
                        if buffer is not None:
                            merged_conversation.append(["USER", buffer])
                            buffer = None
                        merged_conversation.append([speaker, message])
                
                if buffer is not None:
                    merged_conversation.append(["USER", buffer])
                if merged_conversation[-1][0]=="USER":
                    merged_conversation.pop(-1)
                return merged_conversation

            messages = merge_user_messages(messages)
            for i, (role, message) in enumerate(messages):
                if i == 0:
                    assert message, "first message should not be none"
                    assert role == self.roles[0], "first message should come from user"
...
conv_mistral_instruct_LIVE = Conversation(
    system="A chat between a curious user and an artificial intelligence assistant. "
    "The assistant gives helpful, detailed, and polite answers to the user's questions.",
    roles=("USER", "ASSISTANT"),
    version="llama_v2",
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.LLAMA_2_LIVE,
    sep="",
    sep2="</s>",
)
...
conv_templates = {
    ...
    "mpt": conv_mpt,
    "conv_mistral_instruct_LIVE":conv_mistral_instruct_LIVE
}
