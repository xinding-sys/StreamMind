...
def process_video(video_path, processor, aspect_ratio='pad', num_frames=NUM_FRAMES, image_grid=False, sample_scheme='uniform'):
    ...
    if isinstance(video_path, str):
        ...
        # added by lixin4ever, include the support of .webm files from sthsthv2
            ...
            frame_id_list = frame_sample(duration, mode=sample_scheme, local_fps=local_fps)
            # print(duration,sample_scheme)
            # print(frame_id_list,66666666666666666666666666666666)
            # limit the max input frames
            if len(frame_id_list) > MAX_FRAMES:
                frame_id_list = np.linspace(0, duration-1, MAX_FRAMES, dtype=int)
            try:
            ...
            # print(video_data.shape,3435434131334343434435434)
...
# def process_score_video(video_path, processor, aspect_ratio='pad', image_grid=False, sample_scheme='fps',local_fps=2,num_frames=32):
#     def frame_sample(duration, mode='uniform', local_fps=None):
#         if mode == 'uniform':
#             # Calculate the size of each segment from which a frame will be extracted
#             seg_size = float(duration - 1) / num_frames
...
#             raise ImportError(f'Unsupported frame sampling mode: {mode}')
    ...
    if isinstance(video_path, str):
        # decord_vr = VideoReader(uri=video_path, ctx=cpu(0), num_threads=1) 
        # duration, local_fps = len(decord_vr), float(decord_vr.get_avg_fps())
        # frame_id_list = list(range(duration))#将全部的frame都取出来
        # try:
        #     video_data = decord_vr.get_batch(frame_id_list).numpy()
        # except:
        #     video_data = decord_vr.get_batch(frame_id_list).asnumpy()
        # print("video duraion is {}".format(duration))
        # print(44444444444444)
        # NOTE: num_threads=1 is required to avoid deadlock in multiprocessing


        decord_vr = VideoReader(uri=video_path, ctx=cpu(0), num_threads=1) 
        # print(decord_vr,54645464464646446546)
        # duration, local_fps = len(decord_vr), float(decord_vr.get_avg_fps())
        ...
        # print(video_data.shape,46464646646446465)
        # import pdb
        # pdb.set_trace()
    else:
        print("error video_path_{}  !!!!!!!!!!!!!!".format(video_path))
    # import pdb
    # pdb.set_trace()
    if image_grid:
        grid_h = grid_w = math.ceil(math.sqrt(num_frames))
...
def tokenizer_MMODAL_token(prompt, tokenizer, MMODAL_token_index=IMAGE_TOKEN_INDEX, return_tensors=None):
    #做这个而不直接做tokenizer是因为要单独处理句子中的<videos>这个词，所以就按照这个词split然后单独tokenizer，然后把这个词的id插入中间

    # print(MMODAL_INDEX_TOKEN[MMODAL_token_index].lower(),4431313515, prompt,644313146465) #video
    #这个就是按照<video>进行split，然后分别做tokenizer, 得到prompt_chunks:len=2
    #然后再把这两个list合并，同时注意把第二句的第一个id换成-201（video对应的tokenid）
    # print(prompt.split(f'<{MMODAL_INDEX_TOKEN[MMODAL_token_index].lower()}>'),4546445)#这个split相当于把instruction 和 qa分开了
    #eg:["[INST] <<SYS>>\nA chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions.\n<</SYS>>\n\n", 
    #'\nCan you describe the stage in the video? [/INST] Yes, based on the information provided, the stage is adorned with blue lights and at times, a curtain. During one segment, a large screen is shown on stage displaying images. </s>'] 
    
    prompt_chunks = [tokenizer(chunk).input_ids for chunk in prompt.split(f'<{MMODAL_INDEX_TOKEN[MMODAL_token_index].lower()}>')]
#[[1, 733, 16289, 28793, 2087, 18741, 4060, 13, 28741, 10706, 1444, 264, 13903, 2188, 304, 396, 18278, 10895, 13892, 28723, 415, 13892, 5212, 10865, 28725, 10537, 28725, 304, 27057, 11194, 298, 272, 2188, 28742, 28713, 4224, 28723, 13, 28789, 700, 18741, 4060, 13, 13], 
#[1, 28705, 13, 3195, 349, 9615, 297, 272, 3798, 28804, 733, 28748, 16289, 28793, 28705]] len(prompt_chunks[0]):44 len(prompt_chunks[1]):15

    # print(prompt_chunks,135132131,len(prompt_chunks[0]),len(prompt_chunks[1]))
    num_prompt_chunks = len(prompt.split(f'<{MMODAL_INDEX_TOKEN[MMODAL_token_index].lower()}>'))
    # print(num_prompt_chunks,6461654213213)
    ...
    if return_tensors is not None:
        if return_tensors == 'pt':
            return torch.tensor(input_ids, dtype=torch.long)
        raise ValueError(f'Unsupported tensor type: {return_tensors}')
    # print(input_ids,135464135168,len(input_ids))
    #[1, 733, 16289, 28793, 2087, 18741, 4060, 13, 28741, 10706, 1444, 264, 13903, 2188, 304, 396, 18278, 10895, 13892, 28723, 415, 13892, 5212, 10865, 28725, 10537, 28725, 304, 27057, 11194, 298, 272, 2188, 28742, 28713, 4224, 28723, 13, 28789, 700, 18741, 4060, 13, 13, 
    #-201, 28705, 13, 3195, 349, 9615, 297, 272, 3798, 28804, 733, 28748, 16289, 28793, 28705] len=59 和prompt_chunk对比就是-201这个token换了
    return input_ids
