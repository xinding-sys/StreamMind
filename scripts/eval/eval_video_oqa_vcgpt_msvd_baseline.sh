...
CKPT_NAME=finetune_videollama2_baseline
CKPT=work_dirs/videollama2_baseline/finetune_videollama2_baseline
...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        ...
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=1 python3 videollama2/eval/inference_video_oqa_vcgpt.py \
            --model-path ${CKPT} \
